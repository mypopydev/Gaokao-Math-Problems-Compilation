...
\begin{solution}
\begin{enumerate}
\item 当 \(a=1\) 时，\(f(x)=|x+1|-1-x\) 对所有实数 \(x\) 有定义，故定义域为 \(\mathbb R\).
\item 当 \(ax+a\ge0\) 时，\(f(ax)=0\)，不可能等于非零的 \(a\). 当 \(ax+a<0\) 时，\(f(ax)=-2(ax+a)\)，方程给出 \(x=-\frac32\)，且只有在 \(a>0\) 时满足该分支条件. 因此任意 \(a\ne0\) 至多有一个实根，不存在满足“有两个不同实数根”的 \(a\)，故取值范围为空集 \(\varnothing\).
\item 函数可写为
\[
f(x)=\begin{cases}-2(x+a),&x<-a,\\0,&x\ge-a.\end{cases}
\]
它在 \((-\infty,-a)\) 上递减，在 \([-a,+\infty)\) 上为常函数，故对任意实数 \(a\)，函数在定义域 \(\mathbb R\) 上单调不增. 所以第(3)问中 \(a\) 的取值范围为 \(\mathbb R\).
\end{enumerate}
\end{solution}
