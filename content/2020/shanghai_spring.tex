...
\begin{solution}
\begin{enumerate}
\item 因为 \(f(x)=-x\) 为减函数，所以 \(f(x)<f(x-1)\)，故 \(f(x)=-x\) 不具有 \(A\) 性质；\(g(x)=2^x\) 为增函数，所以 \(g(x)>g(x-1)\)，故 \(g(x)=2^x\) 具有 \(A\) 性质.
\item 依题意，对任意 \(t\in(0,1)\)，应有 \(f(x)\geq f(x+t)\)（\(x\geq a\)）. 但对任意可行的 \(a>0\)，取 \(x>\max\{a,1\}\)，则 \(f'(x)=1-\dfrac{1}{x^2}>0\)，从而对充分小的 \(t>0\) 有 \(f(x+t)>f(x)\)，与 A 性质矛盾. 因此不存在满足条件的 \(a\)，取值范围为空集 \(\varnothing\).
