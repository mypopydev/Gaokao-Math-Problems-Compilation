...
\begin{problem}
现定义如下：当 \(x\in(n,n+1)\) 时（\(n\in \N\)），若 \(f(x+1)=f'(x)\)，则称 \(f(x)\) 为延展函数. 已知当 \(x\in(0,1)\) 时，\(g(x)=\e^x\)，且 \(h(x)=x^{10}\)，且 \(g(x)\)，\(h(x)\) 均为延展函数. 结论如下：
...
\item 由圆锥侧面积为 \(\sqrt3\pi\)，底面直径 \(BC=2\)，得底面半径为 \(1\)，母线长为 \(\sqrt3\). 再由 \(PO^2=\left(\sqrt3\right)^2-1^2=2\) 得 \(PO=\sqrt2\)，且 \(PA=\sqrt3\). 建立空间直角坐标系后，可得法向量并计算二面角的余弦值为 \(\frac15\)，故二面角 \(B-PA-C\) 的大小为 \(\pi-\arccos\frac15\).
\end{enumerate}
\end{solution}
